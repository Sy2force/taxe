...
async function extractFromRtf(filePath: string): Promise<{ text: string }> {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      process.on('close', (code) => {
        if (code === 0) resolve();
        else reject(new Error('La conversion du fichier RTF a échoué. Convertissez-le en .docx ou .txt.'));
      });
...
async function extractFromOdt(filePath: string): Promise<{ text: string }> {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      process.on('close', (code) => {
        if (code === 0) resolve();
        else reject(new Error('La conversion du fichier ODT a échoué. Convertissez-le en .docx ou .txt.'));
      });
      process.on('error', (err) => {
...
async function extractFromMarkdown(filePath: string): Promise<{ text: string }> {
  try {
    const text = await fs.readFile(filePath, 'utf-8');
