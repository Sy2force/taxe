...
import { saveDocumentToDb, getAllDocumentsFromDb, saveGeneratedAnswer, getAllGeneratedAnswersFromDb, saveExerciseDocumentToDb, getExerciseDocumentFromDb, saveLawsDocumentToDb, getLawsDocumentFromDb, saveVerificationResult, getAllVerificationResults, clearVerificationResults } from '../services/database.js';
const router = Router();
...
const homeworkQuestions = new Map();
// New simplified architecture: exercise document, laws document, generated answers
let exerciseDocument = null;
let lawsDocument = null;
const generatedAnswers = new Map();
// Upload exercise file (extracts questions)
router.post('/upload-exercise', upload.single('file'), async (req, res) => {
    try {
        if (!req.file)
...
router.post('/upload-laws', upload.single('file'), async (req, res) => {
    try {
        if (!req.file)
            return res.status(400).json({ error: 'Aucun fichier fourni.' });
        const fileType = path.extname(req.file.originalname).toLowerCase();
...
router.post('/generate-answer', async (req, res) => {
    ...
    try {
        const ragResult = await generateAnswerWithRAG(questionText, chunks, getLawsTotalPages(), getOpenAIClient());
...
router.post('/homework-questions', (req, res) => {
    try {
...
router.get('/homework-questions', (req, res) => {
    res.json(Array.from(homeworkQuestions.values()));
});
router.get('/homework-questions/:id', (req, res) => {
...
router.put('/homework-questions/:id', (req, res) => {
    try {
...
router.delete('/homework-questions/:id', (req, res) => {
    const id = parseInt(req.params.id);
...
router.post('/check-final-answer-request', (req, res) => {
    const { text } = req.body;
...
router.post('/compare-question-answer', (req, res) => {
...
router.post('/reduce-lines', (req, res) => {
    ...
    const reducedLines = lines.filter((line) => {
        ...
        if (lines.filter((l) => l.trim() === line.trim()).length > 1) {
...
router.post('/correct-language-only', async (req, res) => {
    const { text } = req.body;
    try {
        if (isOpenAIEnabled()) {
...
router.post('/verify-calculations', (req, res) => {
    const { text } = req.body;
    const calculations = [];
...
router.post('/generate-guided-answer', async (req, res) => {
        ...
        const guidedAnswer = {
            ...
            sections: [
                ...
                },
                {
                    ...
                    content: analysis.usefulPassages.slice(0, 3).map((passage, idx) => `- Extrait ${idx + 1} (page ${passage.page || 'non détectée'}): "${passage.extract.substring(0, 100)}..."`).join('\n')
                },
                {
...
router.post('/verify-homework', (req, res) => {
    ...
    const overallStatus = verification.every((v) => v.status === 'ready')
        ? 'ready'
        : verification.some((v) => v.status === 'incomplete')
            ? 'incomplete'
            : 'needs_improvement';
    res.json({
        overallStatus,
        ...
        summary: {
            ...
            ready: verification.filter((v) => v.status === 'ready').length,
            needsImprovement: verification.filter((v) => v.status === 'needs_improvement').length,
            incomplete: verification.filter((v) => v.status === 'incomplete').length
        }
    });
});
// ─── FINAL DOCUMENT CHECK ──────────────────────────────────────────────────
// Upload final document (student's completed homework)
router.post('/upload-final-document', upload.single('file'), async (req, res) => {
    try {
        if (!req.file)
            return res.status(400).json({ error: 'Aucun fichier fourni.' });
        const fileType = path.extname(req.file.originalname).toLowerCase();
...
router.post('/final-check', async (req, res) => {
    ...
    for (const pair of pairs) {
        ...
        const isOnTopic = questionTerms.some((kw) => answerLower.includes(kw.toLowerCase()));
        ...
        const corrections = [];
        if (!isOnTopic)
            corrections.push('La réponse semble hors sujet par rapport à la question. Vérifiez que vous répondez bien à ce qui est demandé.');
        if (!hasSource)
            corrections.push('Aucune source n\'a été trouvée dans le document de lois pour valider cette réponse.');
        if (!hasRule)
            corrections.push('La règle fiscale applicable n\'est pas clairement mentionnée. Citez la section ou l\'article pertinent.');
        if (!hasFacts)
            corrections.push('La réponse est trop courte. Développez les faits importants et leur application.');
        if (!hasConclusion)
            corrections.push('Ajoutez une conclusion claire précisant l\'événement fiscal, le montant et le taux.');
        if (tooLong)
            corrections.push(`La réponse dépasse 15 lignes (${lines} lignes). Condensez pour respecter la contrainte.`);
        if (!hasCalculation)
            corrections.push('Un calcul chiffré semble nécessaire pour cette question. Vérifiez si des montants sont à calculer.');
        ...
        if (!hasSource)
            status = 'no_source';
        else if (score >= 80 && corrections.length === 0)
            status = 'ready';
        else if (score >= 50)
            status = 'needs_improvement';
        else
    ...
    if (noSource > 0)
        globalStatus = 'Sources manquantes';
    else if (ready < results.length)
        globalStatus = 'À corriger';
...
router.post('/answer-action', async (req, res) => {
    try {
        const { action, questionText, currentAnswer, sources, userQuestion, } = req.body;
        if (!action || !questionText) {
            return res.status(400).json({ error: 'action et questionText requis' });
        }
        const sourcesBlock = (sources || []).slice(0, 5).map((s, i) => `[Source ${i + 1}${s.page ? ` — page ${s.page}` : ''}]\n${s.extract}`).join('\n\n---\n\n') || '(aucune source disponible)';
        const instructions = {
...
router.get('/verification-results', async (req, res) => {
    try {
...
router.delete('/verification-results', async (req, res) => {
    try {
...
router.get('/spectator/all-data', async (req, res) => {
    try {
...
router.get('/spectator/updates', async (req, res) => {
    try {
